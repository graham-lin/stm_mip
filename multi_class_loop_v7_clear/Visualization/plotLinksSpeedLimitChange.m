function [handle_fig,handle_rect,handle_txt] = plotLinksSpeedLimitChange(nodes,links,load,show_labels,fig_num,scale,maxLoad,title_figure)
% V6: specify the function for visualizing how link speed limits are
% changed
...
%set the colours
ctemp=hsv(128);
cmap=colormap(ctemp(1:1:50,:));
% cmap=colormap(handle_ax,ctemp(50:-1:1,:));


minc=min(load);%possible one could also use the minimal positive value of the load  %max(0,min(load));

crec=cmap(ceil(49*(load'-minc+eps)/(maxc-minc+eps))',:);
caxis([minc maxc+0.1]);% modified (+0.1) to allow situation where minc = maxc
